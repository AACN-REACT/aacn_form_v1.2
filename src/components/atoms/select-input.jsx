...
export default function SelectInput(props){
   console.log("PPPROPS:" , props)
    ...
    return <label>{props.label || props.thisfield}<select  onChange={e=>{let ans = props.options.filter(el=>el.name===e.target.value);dispatch(formAction("select",props.thisfield, [e.target.value, ans[0]['key']])) } }>{filterOptions(state,props.options,props.parent)}</select></label>
