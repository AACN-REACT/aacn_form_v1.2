...
function multiMount(el_identifier, component){
    ...
    // Must not use 'for..of' since it breaks ie11
      for(let i =0;i<myNodeList.length;i++){
       //JSON.parse(myNodeList[i].dataset.config)
        ReactDOM.render(<MyComponent compatible={isIE11} config={myNodeList[i].dataset.config[0]!=='{'?require(`./data/${myNodeList[i].dataset.config}.js`):JSON.parse(myNodeList[i].dataset.config)} />, myNodeList[i])
      }
